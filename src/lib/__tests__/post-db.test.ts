...
function buildInMemoryDb() {
  ...
  function exec(sql: string, params: unknown[] = []): Row[] {
    ...
    // SELECT local_image_uri FROM local_posts WHERE id = ?
    if (
      /^SELECT local_image_uri FROM local_posts WHERE id = \?/i.test(trimmed)
    ) {
    ...
    if (/^UPDATE upload_outbox/i.test(trimmed)) {
      const [attemptCount, nextAttemptAt, lastError, id] = params as [
        number,
        number,
        string | null,
        string,
    ...
    if (/^DELETE FROM local_posts WHERE id = \?/i.test(trimmed)) {
      tables.local_posts = tables.local_posts.filter((r) => r.id !== params[0]);
      return [];
    }

    throw new Error(
      `Unhandled SQL (add a case to the mock): ${sql.substring(0, 80)}`,
    );
  }
  ...
  describe("getLocalImageUri", () => {
    it("returns the local_image_uri for an existing post", async () => {
      await insertLocalPost(
        db as never,
        makePost({ local_image_uri: "file:///a.jpg" }),
      );
  ...
  describe("getDueOutboxEntries", () => {
    ...
    it("returns entries whose next_attempt_at <= now for eligible statuses", async () => {
      await insertOutboxEntry(
        db as never,
        makeOutboxEntry({ next_attempt_at: Date.now() - 1 }),
      );
    ...
    it("does not return future-scheduled entries", async () => {
      await insertOutboxEntry(
        db as never,
        makeOutboxEntry({ next_attempt_at: Date.now() + 60_000 }),
      );
      const entries = await getDueOutboxEntries(db as never);
      expect(entries).toHaveLength(0);
    });
