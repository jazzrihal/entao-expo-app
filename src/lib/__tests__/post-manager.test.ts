...
describe("post-manager", () => {
  ...
  describe("saveLocalPost", () => {
    ...
    it("passes a consistent timestamp to insertLocalPost and returns it on the result", async () => {
      ...
      const insertCall = (mockPostDb.insertLocalPost as jest.Mock).mock
        .calls[0];
  ...
  describe("getLocalPosts", () => {
    it("delegates to getLocalPostsByUser", async () => {
      ...
      expect(mockPostDb.getLocalPostsByUser).toHaveBeenCalledWith(
        dbStub,
        "user-a",
      );
    });
  ...
  describe("markSynced", () => {
    it("updates status, deletes outbox entry by local_post_id, then deletes local image file", async () => {
      ...
      (mockPostDb.deleteOutboxByLocalPostId as jest.Mock).mockResolvedValue(
        undefined,
      );
      ...
      expect(mockPostDb.deleteOutboxByLocalPostId).toHaveBeenCalledWith(
        dbStub,
        "post-1",
      );
      expect(mockFileSystem.deleteAsync).toHaveBeenCalledWith(imageUri, {
        idempotent: true,
      });
    });
    ...
    it("does not throw when image has already been deleted", async () => {
      ...
      (mockPostDb.getLocalImageUri as jest.Mock).mockResolvedValue(
        "file:///documents/local-posts/img.jpg",
      );
      mockFileSystem.getInfoAsync.mockResolvedValue({
        exists: false,
        isDirectory: false,
  ...
  describe("deleteLocalPost", () => {
    it("deletes image file and DB row", async () => {
      ...
      expect(mockPostDb.deleteLocalPostRow).toHaveBeenCalledWith(
        dbStub,
        "post-1",
      );
    });
