...
describe("post sharing", () => {
  ...
  it("builds the fixed share message without post content", () => {
    expect(buildPostShareMessage("Alice")).toBe(
      "See Alice’s post on Fotuu.",
    );
  });

  it("validates and normalizes only internal post paths", () => {
    expect(validatePostReturnPath("/post/abc-123")).toBe("/post/abc-123");
    expect(validatePostReturnPath("/(app)/post/abc-123")).toBe(
      "/post/abc-123",
    );
    ...
    expect(validatePostReturnPath("/post/abc?next=https://example.com")).toBeNull();
